...
| `WARN` | Empty-body profile update rejected (`400`) | `UsersService.update` | `Profile update no-op: no fields provided (id: ece2c162-...)` |
...
→ A stale access token (from a deleted account) is being replayed against `DELETE /users/me`. Harmless (idempotent `200`), but worth knowing about — usually a stuck frontend retry.

**Scenario: "the profile page saves, but nothing changes"**

```
WARN  Profile update no-op: no fields provided (id: 9817d3fa-...)
```

→ The frontend sent an empty `PATCH` body (e.g. only sending changed fields and none changed). The API rejected it with `400` and nothing changed. If the frontend expected the avatar to update too — it can't: `avatar` is not editable.
